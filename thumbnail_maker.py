...
FORMAT = "[%(threadName)12s%(asctime)25s%(levelname)7s] - %(message)s"
...
class ThumbnailMakerService(object):
    ...
    def download_image(self, url):
        ...
        with self.dl_sem:
            logging.info('downloading image at URL: %s', url)
            img_filename = urlparse(url).path.split('/')[-1]
            dest_path = f'{self.input_dir}{os.path.sep}{img_filename}'
            urlretrieve(url, dest_path)
            img_size = os.path.getsize(dest_path)
            with self.dl_lock:
                self.downloaded_bytes += img_size

            logging.info('image [%s bytes] saved to: %s', img_size, dest_path)
